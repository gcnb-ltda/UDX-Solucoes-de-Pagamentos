...
def list_payment_accounts(
    ...
    return list(
        db.scalars(
            ...
            .where(PaymentAccount.company_id == actor.company_id)
...
@router.post(
    "/accounts",
    response_model=PaymentAccountOut,
    status_code=status.HTTP_201_CREATED,
)
def create_payment_account(
    payload: PaymentAccountCreate,
    request: Request,
    actor: User = Depends(require_roles(UserRole.admin, UserRole.finance)),
    db: Session = Depends(get_db),
) -> PaymentAccount:
...
def complete_transaction(
    ...
    audit_event(
        db,
        request,
        action="finance.pix.completed_manual",
        actor=actor,
...
def trial_balance(
    ...
    accounts = list(
        db.scalars(
            ...
            .where(LedgerAccount.company_id == actor.company_id)
            .order_by(LedgerAccount.code)
        )
    )
    entries = list(
        db.scalars(
            select(LedgerEntry).where(LedgerEntry.company_id == actor.company_id)
        )
    )
...
def list_audit_logs(
    ...
    return list(
        db.scalars(
            select(AuditLog)
            .where(AuditLog.company_id == actor.company_id)
